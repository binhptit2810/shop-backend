...
const Cart = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 md:px-4 py-4 md:py-6 dark:text-gray-100 pb-20 md:pb-6">
      ...
      {items.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 rounded-sm border border-gray-100 dark:border-gray-700 p-8 md:p-12 text-center flex flex-col items-center justify-center shadow-xs">
          <div className="h-16 w-16 md:h-20 md:w-20 bg-orange-50 dark:bg-gray-900 rounded-full flex items-center justify-center text-primary mb-4">
          ...
          <Link to="/" className="bg-primary hover:bg-primary-hover text-white text-xs font-bold px-6 py-3 rounded-sm transition-all shadow-xs">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 animate-slide-up">
          ...
          <div className="lg:col-span-2 flex flex-col gap-3 md:gap-4">
            {/* Header select all */}
            <div className="bg-white dark:bg-gray-800 rounded-sm p-3 md:p-4 border border-gray-100 dark:border-gray-700 flex items-center justify-between text-xs font-bold text-gray-600 dark:text-gray-300 shadow-xs">
              <div className="flex items-center gap-2.5">
                <input 
                  type="checkbox"
                  checked={isAllSelected()}
                  onChange={handleToggleSelectAll}
                  className="rounded text-primary focus:ring-shopee h-4.5 w-4.5 cursor-pointer accent-shopee"
                />
            ...
            <div className="flex flex-col gap-2.5">
              {items.map(item => {
                ...
                return (
                  <div 
                    key={item.id} 
                    className={`bg-white dark:bg-gray-800 rounded-sm p-3 md:p-4 border transition-all flex items-start md:items-center gap-2.5 md:gap-3 shadow-xs ${
                  ...
                  >
                    <input 
                      ...
                      className="rounded text-primary focus:ring-shopee h-4.5 w-4.5 cursor-pointer accent-shopee flex-shrink-0 mt-1 md:mt-0"
                    />

                    <img 
                      src={imgUrl} 
                      alt={item.productName} 
                      className="w-14 h-14 md:w-16 md:h-16 object-contain rounded-sm border border-gray-105 dark:border-gray-700 bg-gray-50 flex-shrink-0 p-0.5"
                    />

                    <div className="flex-1 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 min-w-0">
                      {/* Title and Price */}
                      <div className="min-w-0 flex-1">
                        <h3 className="font-bold text-xs text-gray-900 dark:text-white line-clamp-2 hover:text-primary cursor-pointer transition-colors leading-snug">
                      ...
                      <div className="flex items-center justify-between sm:justify-end gap-3.5 mt-1 sm:mt-0 flex-shrink-0">
                        <div className="flex items-center border border-gray-200 dark:border-gray-700 rounded-sm overflow-hidden bg-white dark:bg-zinc-900">
                          <button 
            ...
            {activeVouchers.length > 0 && (
              <div className="bg-white dark:bg-gray-800 rounded-sm p-4 border border-gray-100 dark:border-gray-700 shadow-xs mt-1">
                ...
                <div className="flex flex-col gap-2 max-h-40 overflow-y-auto pr-1">
                  {activeVouchers.map(v => (
                    <div 
                      key={v.id} 
                      className="border border-orange-100 dark:border-gray-700 rounded-sm p-3 flex justify-between items-center bg-orange-50/20 dark:bg-gray-950/10 hover:border-orange-200 transition-all text-xs"
                    >
                      <div className="min-w-0 pr-2">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          <span className="font-extrabold text-primary tracking-wider bg-primary/10 px-1.5 py-0.5 rounded text-[9px] uppercase">
                            {v.code}
                          </span>
                      ...
                      <button 
                        ...
                        className="bg-primary text-white text-[10px] font-bold px-3 py-1.5 rounded-sm hover:bg-primary-hover transition-colors focus:outline-none flex-shrink-0"
                      >
                        Áp dụng
                      </button>
                    </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="bg-white dark:bg-gray-800 rounded-sm p-4 md:p-5 border border-gray-100 dark:border-gray-700 shadow-xs flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <input 
                    ...
                    className="flex-1 text-xs px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-sm bg-white dark:bg-gray-900 focus:outline-none focus:border-primary"
                  />
                  <button 
                    onClick={() => handleApplyVoucher(voucherCode)}
                    disabled={checkingVoucher}
                    className="bg-gray-850 dark:bg-gray-700 hover:bg-gray-900 text-white text-xs font-bold px-4 py-2 rounded-sm transition-all disabled:opacity-50 flex-shrink-0 focus:outline-none"
                  >
                    {checkingVoucher ? '...' : 'Áp dụng'}
                  </button>
                </div>
                {appliedVoucher && (
                  <div className="flex items-center justify-between text-[10px] md:text-[11px] bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 px-2 py-1 rounded-sm">
              ...
              <div className="flex flex-col gap-2 pt-1 text-xs text-gray-600 dark:text-gray-300">
                ...
                <div className="flex items-start gap-1 bg-amber-50 dark:bg-amber-950/20 text-[10px] text-amber-600 p-2 rounded-sm mt-1 leading-relaxed">
                ...
                <div className="flex justify-between items-baseline border-t border-gray-150 dark:border-gray-700 pt-3 mt-2 font-bold text-sm">
                  ...
                  <span className="text-base md:text-lg text-primary font-black">
              ...
              <button 
                ...
                className="hidden lg:flex w-full bg-primary hover:bg-primary-hover text-white font-bold py-3 px-4 rounded-sm transition-all text-xs items-center justify-center gap-2 shadow-xs mt-1.5 focus:outline-none"
              >
                <CreditCard size={15} />
              ...
              <Link 
                to="/" 
                className="text-center text-[11px] text-gray-400 hover:text-primary transition-colors mt-1 font-semibold"
              >
      ...
      {items.length > 0 && (
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white dark:bg-gray-950 border-t border-gray-100 dark:border-gray-900 px-4 py-3.5 shadow-lg flex items-center justify-between gap-3 backdrop-blur-md bg-opacity-95 dark:bg-opacity-95 animate-fade-in">
          ...
          <button 
            onClick={handleCheckoutRedirect}
            className="bg-primary hover:bg-primary-hover text-white font-bold py-2.5 px-4 rounded-sm transition-all text-xs flex items-center gap-1.5 shadow-xs focus:outline-none flex-shrink-0"
          >
            <CreditCard size={14} />
