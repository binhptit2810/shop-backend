  ...
  return (
    <div>
      <div style={{ marginBottom: '20px' }}>
        <h2 style={{ margin: 0, fontSize: '22px', fontWeight: 800, color: '#1a1a2e' }}>Tin nhắn</h2>
        <p style={{ margin: '4px 0 0', color: '#888', fontSize: '14px' }}>Trao đổi với người mua về đơn hàng</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '16px', height: 'calc(100vh - 220px)', background: '#fff', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 2px 16px rgba(0,0,0,0.08)' }}>
        ...
        <div style={{ width: '280px', flexShrink: 0, borderRight: '1px solid #f1f5f9', overflowY: 'auto', background: '#fafafa' }}>
          <div style={{ padding: '16px', borderBottom: '1px solid #f1f5f9' }}>
            <h4 style={{ margin: 0, fontSize: '13px', fontWeight: 600, color: '#888', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Đơn hàng ({orders.length})</h4>
          </div>
            ...
            return (
              <div
                ...
                style={{
                  padding: '14px 16px',
                  cursor: 'pointer',
                  background: isActive ? 'rgba(233,69,96,0.05)' : 'transparent',
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px' }}>
                  <span style={{ fontSize: '14px', fontWeight: 700, color: isActive ? '#e94560' : '#1a1a2e' }}>Đơn #{order.id}</span>
                  <span style={{ fontSize: '11px', color: '#aaa' }}>{order.createdAt ? new Date(order.createdAt).toLocaleDateString('vi-VN') : ''}</span>
                </div>
                <div style={{ fontSize: '12px', color: '#888' }}>{statusLabels[order.status] || order.status}</div>
        ...
        {selectedOrderId ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ padding: '16px 20px', borderBottom: '1px solid #f1f5f9', background: '#fff', display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, #e94560, #c0392b)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: '14px' }}>
                {selectedOrder?.username?.[0]?.toUpperCase() || '?'}
              </div>
              <div>
                <div style={{ fontSize: '14px', fontWeight: 700, color: '#1a1a2e' }}>Người mua – Đơn #{selectedOrderId}</div>
                <div style={{ fontSize: '12px', color: '#888' }}>{selectedOrder?.shippingAddress?.slice(0, 50)}...</div>
              </div>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px', background: '#f8fafc', display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {messages.length === 0 ? (
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#888' }}>
                  ...
                  <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#bbb' }}>Hãy bắt đầu cuộc trò chuyện!</p>
                </div>
              ) : messages.map(msg => {
                const isMine = msg.senderId === currentUserId;
                return (
                  <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isMine ? 'flex-end' : 'flex-start' }}>
                    <div style={{ maxWidth: '65%', background: isMine ? 'linear-gradient(135deg, #e94560, #c0392b)' : '#fff', color: isMine ? '#fff' : '#1a1a2e', padding: '10px 16px', borderRadius: isMine ? '20px 20px 4px 20px' : '20px 20px 20px 4px', fontSize: '14px', lineHeight: '1.5', boxShadow: '0 2px 8px rgba(0,0,0,0.08)', wordBreak: 'break-word' }}>
                      {msg.content}
                    </div>
            ...
            <form onSubmit={handleSend} style={{ padding: '16px 20px', borderTop: '1px solid #f1f5f9', display: 'flex', gap: '10px', alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ width: '44px', height: '44px', borderRadius: '50%', border: 'none', background: input.trim() && !sending ? 'linear-gradient(135deg, #e94560, #c0392b)' : '#e2e8f0', color: '#fff', cursor: input.trim() && !sending ? 'pointer' : 'not-allowed', fontSize: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s', flexShrink: 0 }}
              >
                {sending ? '⏳' : '➤'}
              </button>
            </form>
          ...
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#888' }}>
            <div style={{ textAlign: 'center' }}>
              ...
              <p>Chọn một đơn hàng để bắt đầu chat</p>
            </div>
          </div>
        )}
      </div>
